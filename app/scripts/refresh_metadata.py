            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    for card_data in data:
                        db_card = next(
                            (c for c in batch if c.scryfall_id == card_data["id"]), None
                        )
                        if db_card:
                            # Update Metadata
                            ...
                            db_card.current_price = float(
                                prices.get("usd") or prices.get("usd_foil") or 0
                            )

                    session.commit()
                    print(
                        f"Hydrated batch {i//75 + 1} (Up to card {min(i+75, len(cards))})"
                    )
                else:
