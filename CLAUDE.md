...
- v3.13.5: Fix Compass corner gaps — side players (`l`/`r`) now span full height via repeated grid-area names (`"l top r" "l bot r"`); top/bot fill the center column between them; Triangle similarly collapsed to `"l top r"` single-row so all three cells are full-height — **committed**
