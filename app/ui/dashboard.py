...
def remove_inventory_item(item_id, count_to_remove, action_type="REMOVE"):
    ...
    with Session(engine) as session:
        ...
        # Break long line for log details
        ...
        for index, row in df.iterrows():
            ...
            try:
                ...
                if not img_url and "card_faces" in raw_data:
                    img_url = (
                        raw_data["card_faces"][0].get("image_uris", {}).get("normal")
                    )
            ...
            if existing:
                ...
                if location_type == "Drawer":
                    ...
                    new_key = natural_key(coll_num)
                    for item in existing_inv:
                        item_coll = natural_key(item.card.collector_number)
                        # Complex sorting logic split for length
                        if (item.card.set_code > set_code) or (
                            item.card.set_code == set_code and item_coll > new_key
                        ):
                            pos = item.position
                            break
                        pos = item.position + 1
            ...
            log_msg = f"Target: {location_type}"
            session.add(
                TransactionLog(
                    action="IMPORT",
                    card_name=card_name,
...
with st.sidebar:
    ...
    with Session(engine) as session:
        val_stmt = select(func.sum(Card.current_price * Inventory.quantity)).join(
            Inventory
        )
    ...
    if page == "Full Collection":
        ...
        with st.expander("🛠️ Bulk Remove / Sell Cards"):
            ...
            if rem_q:
                results = session.exec(
                    select(Card).where(col(Card.name).ilike(f"%{rem_q}%"))
                ).all()
                for card in results:
                    for inv in card.inventory_items:
                        ...
                        with r1:
                            st.write(
                                f"**{card.name}** | {inv.finish.upper()} | D:{inv.drawer} P:{inv.position}"
                            )
                        with r2:
        ...
        for idx, card in enumerate(cards):
            with cols[idx % 6]:
                ...
                st.caption(
                    f"#{card.collector_number} ({card.set_code.upper()}) | {price_disp}"
                )
        ...
        with c2:
            ...
            if st.button("Commit Pull") and name:
                for e in st.session_state.deck_draft:
                    pull_stmt = select(Inventory).where(
                        Inventory.card_id == e["id"],
                        Inventory.location_type == "Drawer",
                    )
                    inv = session.exec(pull_stmt).first()
                    if inv:
        ...
        p_stmt = (
            select(Inventory)
            .where(not col(Inventory.is_placed))
            .order_by(Inventory.drawer, Inventory.position)
        )
        pending = session.exec(p_stmt).all()
        ...
        # Fixed ambiguous variable name 'l'
        st.dataframe(
