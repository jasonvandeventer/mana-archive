...
def _query_inventory(
    ...
    with Session(get_engine()) as session:
        ...
        rows = session.exec(base_stmt).all()
    ...
    for inv, card in rows:
        ...
        row["effective_price"] = _effective_inventory_price(row)
        result.append(row)

    filtered = []
    for row in result:
        effective_price = row["effective_price"]
        if min_price > 0 and (effective_price is None or effective_price < min_price):
            continue
        if max_price < 9999 and (effective_price is None or effective_price > max_price):
            continue
        filtered.append(row)

    def _sort_key(row: dict):
        price = row["effective_price"]
        price_for_desc = price if price is not None else -1.0
        price_for_asc = price if price is not None else float("inf")

        if sort_by == "Name":
            return (row["name"].lower(), row["drawer"], row["position"])
        if sort_by == "Set Code":
            return (row["set_code"].lower(), str(row["collector_number"]))
        if sort_by == "Price ↓":
            return (-price_for_desc, row["name"].lower())
        if sort_by == "Price ↑":
            return (price_for_asc, row["name"].lower())
        if sort_by == "CMC":
            return (
                row["cmc"] if row["cmc"] is not None else float("inf"),
                row["name"].lower(),
            )
        if sort_by == "Type":
            return ((row["type_line"] or "").lower(), row["name"].lower())
        if sort_by == "Quantity ↓":
            return (-row["quantity"], row["name"].lower())
        return (row["drawer"], row["position"])
...
def render() -> None:
    ...
    with st.expander("Filters & Sort", expanded=True):
        ...
        with row2_c1:
            sort_by = st.selectbox("Sort by", list(_SORT_OPTIONS))
